          ...
          <div className="animate-slide-up bg-white rounded-3xl shadow-xl overflow-hidden min-h-[70vh]">
            ...
            <div className="p-8">
              ...
              <div className="mt-12 pt-8 border-t border-rose-50 flex justify-center">
                <Heart className={heartClicked ? "text-rose-200 fill-rose-200" : "text-rose-100"} size={32} onClick={() => setHeartClicked(!heartClicked)} />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-rose-900/20 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <form onSubmit={handleAddEvent} className="bg-white w-full max-w-md rounded-[2rem] p-8 shadow-2xl animate-in zoom-in-95">
            ...
            <div className="space-y-4">
              ...
              <textarea placeholder="The Story..." rows="4" className="w-full p-3 rounded-xl bg-rose-100 outline-none" onChange={(e) => setNewEvent({...newEvent, story: e.target.value})} required />
      ...
      {editForm && (
        <div className="fixed inset-0 bg-rose-900/20 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <form onSubmit={handleEditEvent} className="bg-white w-full max-w-md rounded-[2rem] p-8 shadow-2xl animate-in zoom-in-95">
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                <input placeholder="Date (February 1)" className="flex-1 p-3 rounded-xl bg-rose-100 outline-none"  value={newEvent.date} onChange={(e) => setNewEvent({...newEvent, date: e.target.value})} required />
                <input placeholder="Year" className="w-24 p-3 rounded-xl bg-rose-100 outline-none" value={newEvent.year} onChange={(e) => setNewEvent({...newEvent, year: e.target.value})} required />
              </div>
              <input placeholder="Short Description" className="w-full p-3 rounded-xl bg-rose-100 outline-none" value={newEvent.shortDesc} onChange={(e) => setNewEvent({...newEvent, shortDesc: e.target.value})} required />
              <input placeholder="Full Title" className="w-full p-3 rounded-xl bg-rose-100 outline-none" value={newEvent.fullTitle} onChange={(e) => setNewEvent({...newEvent, fullTitle: e.target.value})} required />
              <textarea placeholder="The Story..." rows="4" className="w-full p-3 rounded-xl bg-rose-100 outline-none" value={newEvent.story} onChange={(e) => setNewEvent({...newEvent, story: e.target.value})} required />
              <div className="relative">
                <input type="file" accept="image/*" onChange={(e) => setSelectedFile(e.target.files[0])} className="hidden" id="file-upload" />
                <label htmlFor="file-upload" className="flex items-center justify-center gap-2 w-full p-3 rounded-xl bg-rose-100 text-rose-600 cursor-pointer hover:bg-rose-200 transition-colors hover:scale-105">
                  <ImageIcon size={18} /> {selectedFile ? selectedFile.name : "Upload Photo"}
